...
# Re-binding the delegate is what schedules the expensive rebuild; a check
# placed after it defers nothing.
...
# --- Rule 2 (54ed1974): the defer branch cancels the hide countdown.
# Every task EXIT arms hidePreviewWinTimer and only show() used to cancel
# it; a deferred switch that skips the cancel lets the dialog hide under
# the pointer mid-scrub (previews "stopped appearing", reproduced live).
...
# --- Rule 3 (4b533b8d): the settle timer adopts DIRECTLY (prepare + show).
# Re-entering showPreviewWindow() counts the re-entry as a fresh switch
# request and re-defers forever.
...
# --- Rule 4 (4b533b8d): settle interval stays BELOW the burst threshold.
# A steady sweep whose crossings are slower than the timer degenerates back
# to one rebuild stall per icon (the first cut had exactly this bug at
# 120ms vs 140ms crossings).
interval=$(grep -A2 "id: previewSwitchSettleTimer" "$tasksmain" | grep -oE "interval: [0-9]+" | grep -oE "[0-9]+" || true)
threshold=$(grep -oE "switchBurstThreshold: [0-9]+" "$tasksmain" | grep -oE "[0-9]+" || true)
if [[ -z "$interval" || -z "$threshold" || "$interval" -ge "$threshold" ]]; then
    bad "previewSwitchSettleTimer.interval ($interval) must be < switchBurstThreshold ($threshold)" \
        "4b533b8d: interval >= threshold degenerates a steady sweep back to one stall per icon"
fi
